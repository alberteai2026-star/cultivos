...
- **Infra local:** MySQL local o Docker Compose (`docker-compose.mysql.yml`)
...
> Esto crea MySQL 8.0 en `localhost:3306` con base `cultivos` y usuario `cultivos`.
...
- `GET /api/v1/health/ready` (readiness probe; retorna `200` cuando DB está disponible y `503` cuando no lo está)
- `GET /api/v1/health/health` (alias legado **deprecado** para compatibilidad retroactiva)
...
- `GET /api/v1/reports/templates` (listado paginado de plantillas por finca/tipo/estado activo)
- `POST /api/v1/reports/templates` (crea plantilla reutilizable de reportes por finca)
- `GET /api/v1/reports/templates/{template_id}` (detalle de plantilla autorizada)
- `PATCH /api/v1/reports/templates/{template_id}` (actualiza nombre, formato, filtros y estado activo de plantilla)
- `POST /api/v1/reports/templates/{template_id}/generate` (genera exportación nueva desde plantilla activa)
...
- `GET /api/v1/ai/insights/recommendations` (backlog de recomendaciones priorizadas y pendientes)
- `GET /api/v1/ai/insights/summary` (resumen agregado por tipo/estado/prioridad)
- `GET /api/v1/ai/insights/yield-forecast` (proyección de rendimiento por lote combinando histórico + señal IA)
- `GET /api/v1/ai/insights/predictive-alerts` (alertas predictivas de riesgo con umbral de confianza)
...
- `GET /api/v1/audit/summary` (resumen agregado por módulo/acción con filtros forenses y rango temporal)
...
- `GET /api/v1/audit/exports/{export_id}/verify-signature` (valida integridad de firma hash de la exportación)
...
## Migración inicial
Aplicar con runner automático:

```bash
python scripts/migrate.py
```

Opcional (manual): aplicar SQL en orden numérico dentro de `db/migrations/`.
