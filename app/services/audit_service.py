...
from app.schemas.audit import AuditExportSignatureVerifyOut, AuditSummaryItem, AuditSummaryResponse
...
class AuditService:
    ...
    def summary_for_user(
        ...
        *,
        ...
        items = [AuditSummaryItem(module=m, action=a, total=t) for m, a, t in rows]
        return AuditSummaryResponse(total_groups=len(items), items=items)
    ...
    def verify_export_signature_for_user(self, *, user: User, export_id: int) -> AuditExportSignatureVerifyOut:
        export = self.audit.get_export_by_id(export_id)
        if not export:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Exportación de auditoría no encontrada')
        if export.user_id != user.id:
        ...
        parsed_filters = self._safe_load_json(export.filters_json) or {}
        expected = hashlib.sha256(json.dumps(parsed_filters, sort_keys=True).encode('utf-8')).hexdigest()
        current = export.signature
        return AuditExportSignatureVerifyOut(
            export_id=export.id,
            valid=bool(current) and current == expected,
