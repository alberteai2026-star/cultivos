...
def get_audit_summary(
    ...
    return AuditService(db).summary_for_user(
        user=user,
        farm_id=farm_id,
        module=module,
        action=action,
...
@router.get('/exports/{export_id}/verify-signature', response_model=AuditExportSignatureVerifyOut)
def verify_audit_export_signature(export_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    return AuditService(db).verify_export_signature_for_user(user=user, export_id=export_id)
